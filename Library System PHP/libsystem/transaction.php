...
<?php
	...
	$stuid = $student['id'];
	$sql = "SELECT * FROM borrow LEFT JOIN books ON books.id=borrow.book_id WHERE student_id = '$stuid' AND borrow.status = '0' ";
	$action = '';
			        					...
			        					echo "
			        						<tr>
												...
												<?php if($action == ''){
													...
													if(strtotime($row['date_borrow']) > strtotime($row['due_date'])){
														echo "<td>Overdue</td>";
													  } 
													  else{
														echo "<td>Onhand</td>";
